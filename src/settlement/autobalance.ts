...
export function autoBalanceToZero(entries: BalanceEntry[]): AutoBalanceResult {
    const net_before = sum(entries);

    if (net_before === 0) {
        return { adjusted: entries, net_before, applied_net: 0, note: null };
    }
    ...
    if (net_before > 0) {
        ...
        return {
            ...
            net_before,
            applied_net: 0,
            note: `Net is +${net_before} but there are no losers to absorb extra.`
        };
        }
        ...
        adjusted,
        net_before,
        applied_net: net_before,
        note: `Auto-balanced +${net_before} extra by distributing to losers proportionally.`
        };
    } 
    else {
        // Missing money: take from winners proportional to their winnings
        const missing = -net_before;
        ...
        return {
            ...
            net_before,
            applied_net: 0,
            note: `Net is ${net_before} but there are no winners to deduct from.`
        };
        }
        ...
        net_before,
        applied_net: missing,
        note: `Auto-balanced ${net_before} missing by deducting from winners proportionally.`
        };
    }
}
